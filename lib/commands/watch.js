...
var _watch = function(args){
  ...
  function watch_function(file){
    ...
    if (fs.existsSync(file.fullPath)){
      ...
      var graphedFiles = grapher.getGraph(file)
      if (graphedFiles) {
        graphedFiles.forEach(function(f) {
          compile_single_file(f);
        });

        return compile_single_file(file.fullPath);
      }
